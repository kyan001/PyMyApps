...
if cit.get_input("Update IP list from CloudFlare? (y/n)").lower() == 'y':
...
def pping_ipn(ip_network: str):
    ...
    for ip in ipn:
        threading.Thread(target=pping_ipa, args=(ip,)).start()


def main():
    for cloudflare_ip_network in CLOUDFLARE_IP_NETWORKS:
        ipn = ipaddress.ip_network(cloudflare_ip_network)
        threading.Thread(target=pping_ipa, args=(random.choice([ip for ip in ipn.hosts()]),)).start()
